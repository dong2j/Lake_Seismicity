...
for j = 1:nL
    ...
    dt=diff(time);
    dstorage=diff(storage);
    storage=dstorage./dt;
    time=0.5*(time(1:end-1)+time(2:end));
...
xlabel('d_{max} (km)'); ylabel('EF_{storage rate}=\lambda_{inc}/\lambda_{dec}');
...
yline(1,'--','Color',[0.4 0.4 0.4]); grid on;
xlabel('d_{max} (km)'); ylabel('EF_{storage rate}');
...
fname = ['./Processed_Data/EQ_EF_Storage_Rate_Jackknife_Null_depth',num2str(maxDepth_km),'km.txt'];
